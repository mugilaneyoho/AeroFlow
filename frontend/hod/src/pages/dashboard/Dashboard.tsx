      ...
      <div className='flex flex-col p-2 gap-2 pt-6 sm:flex-row sm:overflow-x-auto mb-2'>
        <div className='p-2 w-full rounded-[10px] shadow-[0px_0px_10px_0px_#00000040] min-w-[320px]' >
          <div className='flex justify-between items-center'>
            ...
            <div className='p-2 rounded-[10px] shadow-[0px_0px_14px_0px_#2D216140] ' style={{ backgroundColor: COLORS.primary_blue, color: COLORS.secondary_white }}>
              {onlineClassCount} classes</div>
          </div>

          <div className='p-2 '>
            {onlineClass?.length > 0 ? (
              onlineClass.map((data: any, index: number) => (
                <ClassCard
                    key={index}
                    topic={data.subject}
                    batch={data.batch_name}
                    time={`${new Date(data?.start_time).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })} - ${new Date(data?.end_time).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}`}
                    name={data?.staff?.staff_name}
                    student={data?.total_student || 0}
                    COLORS={COLORS}
        ...
        <div className='p-2 w-full rounded-[10px] shadow-[0px_0px_10px_0px_#00000040] min-w-[320px]'>
          <div className='flex justify-between items-center '>
            ...
            <div className='p-2 rounded-[10px] shadow-[0px_0px_14px_0px_#2D216140]' style={{ backgroundColor: COLORS.primary_blue, color: COLORS.secondary_white }}>
              {offlineClassCount} classes</div>
          </div>

          <div className='p-2'>
            {
              offlineClass?.length > 0 ? (
                offlineClass.map((data: any, index: number) => (
                  <ClassCard
                    key={index}
                    topic={data.subject}
                    batch={data.batch_name}
