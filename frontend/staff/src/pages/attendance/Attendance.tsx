...
const Attendance = () => {
  ...
  const handleCheckboxChange = (uuid: string) => {
    ...
    const findIndex = studentList.findIndex((item)=>item.studentId === uuid)

    console.log(findIndex,"findIndex")

    if (findIndex >= 0) {
     return setStudentList(prev=>prev.splice(findIndex,1))
    }

    setStudentList((prev)=>[...prev,{studentId:uuid,status:'PRESENT'}])
  };
    ...
    <div className="space-y-4">
...

      {
        attendanceData?.map((data:any,index:number)=>(
          <div key={index}>
            <AttendanceCard classData={data?.classData} batchData={data?.batchData}  handleCheckboxChange={handleCheckboxChange} handleSubmit={handleSubmit}/>
          </div>
        ))
      }
