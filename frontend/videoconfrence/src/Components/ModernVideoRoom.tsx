...
const ModernVideoRoom = ({ sessionId, participantName, role, classId, token }:{sessionId:string,participantName:string,role:string, classId: string, token: string}) => {
  ...
  useEffect(() => {
    ...
    const joinSession = async () => {
      try {
        const response = await fetch('http://localhost:3000/openvidu/api/classroom/join', {
          method: 'POST',
          headers: { 
        ...
        mySession.on('streamDestroyed', (event) => {
          if (isMounted) {
            setParticipants(prev => prev.filter((p:any) => p.id !== event.stream.streamId));
          }
        });
