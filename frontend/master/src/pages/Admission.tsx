...
import { adminCard, adminData } from '../dummyData/admissiondata'
      ...
      <div className='mt-4 p-2 bg-[#FFFFFF] shadow-[0_0_15px_rgba(0,0,0,0.1)] rounded-xl'>
         ...
         <div className='grid sm:grid-cols-2 md:grid-cols-2 xl:grid-cols-4 gap-5 p-2'>
            {adminData.map((data,index)=>{
              return(
                <div key={index} className='bg-[#FFFFFF] shadow-[0_0_15px_rgba(0,0,0,0.1)] p-4 rounded-xl border border-[#008BBF]'>
               <div className='flex flex-col justify-center items-center gap-1 mb-3'>
                  <img src={profileimg} />
                    <div className='font-bold text-lg'>{data.name}</div>
                  <div className='text-gray-600'>{data.dept}</div>
                </div> 
